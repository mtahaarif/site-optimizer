/**
 * Database schema — Postgres (Vercel Postgres / Neon).
 *
 * Migrations are forward-only and idempotent, tracked in `schema_migrations`.
 * Each entry runs once inside its own transaction, so an existing database
...
export const MIGRATIONS: string[] = [
  ...
  // ---- 4: crawl history ---------------------------------------------------
  `
  -- Crawl reports move into the same database so score history is queryable
  -- alongside uptime and rankings. The full AuditReport stays as a JSON blob
  -- because it is read whole and never queried field-by-field.
  CREATE TABLE crawls (
    id             TEXT    PRIMARY KEY,
    site_id        INTEGER REFERENCES sites(id) ON DELETE CASCADE,
    created_at     BIGINT  NOT NULL,
    ...
    warnings       INTEGER,
    is_next        INTEGER,
    -- Kept as TEXT rather than JSONB: matches every other JSON blob column in
    -- this schema (serp_features, strengths, fixes), so one INSERT parameter
    -- shape works everywhere with no per-column type-cast special-casing.
  ...
  CREATE TABLE ga4_metrics (
    ...
    avg_duration_sec DOUBLE PRECISION NOT NULL DEFAULT 0.0,
    fetched_at       BIGINT  NOT NULL,
  ...
  // ---- 9: generic key/value cache ------------------------------------------
  `
  -- A small generic cache for anything that used to be a local disk file —
  -- currently just PageSpeed Insights responses. Vercel's filesystem is
  -- read-only outside /tmp and not shared across invocations, so a cache that
  -- must survive between requests has to live here instead.
  ...
  // ---- 10: in-flight crawl job state ---------------------------------------
  `
  -- A crawl runs detached from the request that started it, and the client
  -- polls for progress. On a single long-lived process an in-memory map would
  -- do; on Vercel the poll can land on a different serverless instance than
  -- the one running the crawl, so progress has to be visible from anywhere.
  CREATE TABLE IF NOT EXISTS crawl_jobs (
    id          TEXT    PRIMARY KEY,
    start_url   TEXT    NOT NULL,
    status      TEXT    NOT NULL,   -- queued | running | done | error
    progress    TEXT    NOT NULL,   -- JSON: { phase, crawled, queued, total, currentUrl, message }
    error       TEXT,
    created_at  BIGINT  NOT NULL,
    finished_at BIGINT
  );
